...
const result1 = numbersEx1.reduce(
...
// Name of the params could change but the position should always be the same!
numbersEx1.reduce((acc, val, _, array) => {
  console.log('Our array is', array);
  return acc + val;
});

console.log('Result for ex. 1', result1);

// Exercise 2:
// Find the maximum value in the given array. (Hint: Use -Infinity to compare values to)
const numbersEx2 = [8, 3, 11, 6, 4];
const max = numbersEx2.reduce((accumulator, currentValue) => {
...
// The currentValue is the current item from the array. The accumulator is an object that keeps track of how many times each item has appeared in the array.
//If the item already exists in the object, we increment its count by 1. If it doesn't exist, we initialize its count to 1. 

// Exercise 4:
// Calculate the average of all numbers in the given array.
const numbersEx4 = [10, 20, 30, 40, 50];
const average = numbersEx4.reduce((accumulator, currentValue, _, array) => {
  if (array.indexOf(currentValue) === array.length - 1) {
...
// This code uses reduce() to collect only unique values from the array. It initializes the accumulator as an empty array.
// For each currentValue in the numbers array, it checks if the accumulator already includes that value.
// If it doesn't, the value is pushed into the accumulator array. Finally, .length is used to count how many unique elements were found.
