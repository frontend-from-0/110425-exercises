...
console.log('Result for ex. 2', numbersEx2.reduce((accumulator, currentValue) => {
...
const elementCounts = elements.reduce((accumulator, currentValue) => {
  if (accumulator[currentValue]) { // checks if the current letter is already a key in our object. 
    accumulator[currentValue]++; // if it exists it increments the current value of the letter. 
  } else {
    accumulator[currentValue] = 1; // if it doesn't exist it creates a key and sets its value to 1. 
  }
  return accumulator;
}, {}); // {} tells JS to start our accumulator as an empty object. 
